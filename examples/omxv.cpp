...
#define MLXFPS 8
#define FRAME_TIME_MICROS (1000000/MLXFPS)
...
void setupMLX90640()
{
  ...
  case 64:
    ...
    break;
  default:
    printf("Unsupported framerate: %d", MLXFPS);
    return 1;
  }
  MLX90640_SetChessMode(MLX_I2C_ADDR);

  MLX90640_DumpEE(MLX_I2C_ADDR, eeMLX90640);
  MLX90640_ExtractParameters(eeMLX90640, &mlx90640params);
  printf("setup MLX90640 done\n");
}
...
void updateMLX90640()
{
  static uint16_t framedat[834];
  MLX90640_GetFrameData(MLX_I2C_ADDR, framedat);
  MLX90640_InterpolateOutliers(framedat, eeMLX90640);

  float eTa = MLX90640_GetTa(framedat, &mlx90640params);
  float emissivity = 1;
  MLX90640_CalculateTo(framedat, &mlx90640params, emissivity, eTa, mlx90640To);
...
int main (){
  ...
  long nowtv = spec.tv_sec*1000 + spec.tv_nsec/1.0e6;
  long endtv = nowtv + 30000;
  ...
  auto startch = std::chrono::system_clock::now();
  while (1){
    encoder_output_buffer->nFlags = 0;
    encoder_output_buffer->nFilledLen = 0;
    ...
    if (!firstUse)
    {
      // fprintf(stderr, "here we are is the buffer %i empty?\n", renderbuf);
      ...
      // fprintf(stderr, "here we are the buffer %i is empty = %x\n", retrieved_events);
    }
    ...
    camera_output_buffer->nFilledLen = 0;
    ...
    render_input_buffer[renderbuf]->nFlags = 0;
    ...
    auto endch = std::chrono::system_clock::now();
    auto elapsed = std::chrono::duration_cast<std::chrono::microseconds>(endch - startch);
    ...
    clock_gettime (CLOCK_MONOTONIC, &spec);
    printf("running for %g\n", spec.tv_sec + spec.tv_nsec/1.0e9- endtv/1000.);
    if (spec.tv_sec*1000 + spec.tv_nsec/1.0e6 >= endtv) break;
  }
#else
  while (1){
    //Get the buffer data
